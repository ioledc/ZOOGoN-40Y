...
The pipeline consists of seven steps, each implemented as a standalone function.
...
the remaining steps run on a regular schedule.
...
`format_to_dc()` downloads the merged dataset from SharePoint and builds the
three Darwin Core tables (Event, Occurrence, eMoF), making the data ready for
submission to international repositories such as OBIS and EMODnet Biology.

See the [Darwin Core vignette](darwin-core.html) for details on the table
structure and standards applied.

## Step 5: Build Archive

`format_to_DC_archive()` assembles the Darwin Core tables into a submission-ready
zip archive with embedded EML metadata, and uploads it to SharePoint. This is the
file that is registered with and downloaded by GBIF or EMODnet Biology.
