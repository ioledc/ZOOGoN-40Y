...
`render_report()` renders the Quarto monitoring report using the latest
preprocessed survey data and saves it as a dated HTML file. In the automated
pipeline this runs in parallel with the archive build, so both outputs are
