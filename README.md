...
# 2. Ingest field surveys from KoboToolbox
ingest_surveys()

# 3. Preprocess and standardize survey data
preprocess_surveys()

# 4. Merge legacy + ongoing data into an analysis-ready dataset
format_to_tidy()

# 5. Convert tidy data to Darwin Core format (Event, Occurrence, eMoF)
format_to_dc()

# 6. Build Darwin Core Archive with EML metadata and upload
...
- Build a Darwin Core Archive and EML with `format_to_DC_archive()`
  (writes the zip and uploads it to SharePoint). This reads the Darwin
  Core tables produced by `format_to_dc()`.
...
- Test registration: use `register_gbif_dataset_test()` — same pattern
  targeting the GBIF-Test sandbox; set the `GBIF_TEST_*` environment
